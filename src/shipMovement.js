...
export let shipMovement = (() => {
  ...
  function setUpShips(player) {
    ...
    player.board.placeShip(
      ...
      cordinates1,
      createShips().battleship,
    );

    let cordinates2 = new shipClasses.coordinates(
      getRandomInt(0),
      getRandomInt(9),
    );
    player.board.placeShip(
    ...
    let cordinates6 = new shipClasses.coordinates(
      getRandomInt(6),
      getRandomInt(6),
    );
    player.board.placeShip(
    ...
    for (let i = 0; i < 2; i++) {
      let cordinates4 = new shipClasses.coordinates(
        getRandomInt(3 + i),
        getRandomInt(9 - i),
      );
      player.board.placeShip(
    ...
    for (let index = 0; index < 2; index++) {
      let cordinates3 = new shipClasses.coordinates(
        getRandomInt(6 + index),
        getRandomInt(10 - index),
      );
      player.board.placeShip(
        player.board.playerBoard,
        cordinates3,
        createShips().destroyer,
      );
  ...
  function exchangingAttacks() {
    playerTwo.board.receiveAttack(
      new shipClasses.coordinates(
        prompt("attack coordinate"),
        prompt("attack coordinate"),
      ),
    );
