...
let shipClasses = (() => {
  class Ship {
    ...
    shipStruck(coordinates, board) {
      ...
      if (
        board[coordinates.xCoordinate][coordinates.yCoordinate] ===
        this.shipNumberEquivalent + "8"
      ) {
        this.hit();
        this.#addtodeadzone(coordinates);
        return true;
  ...
  class Gameboard extends Ship {
    ...
    addCoordinates(coordinates) {
      this.#alreadyExistingAttacks[this.#alreadyExistingAttacks.length] =
        coordinates;
    }
    ...
    #placeEasternShip(opponent, coordinates, Ship) {
      ...
      Ship.occupationGrid = new shipRange(
        coordinates.yCoordinate,
        coordinates.yCoordinate + Ship.length,
      );
      for (let i = 0; i < Ship.length; i++) {
        board[coordinates.xCoordinate][coordinates.yCoordinate + i] =
          Ship.shipNumberEquivalent;
      }
    ...
    #placeNorthenShip(opponent, coordinates, Ship) {
      ...
      if (Ship.length > coordinates.xCoordinate) {
        coordinates.xCoordinate =
          coordinates.xCoordinate + (Ship.length - coordinates.xCoordinate) - 1;
      }
      Ship.occupationGrid = new shipRange(
        coordinates.xCoordinate - Ship.length,
        coordinates.xCoordinate,
      );

      for (let i = 0; i < Ship.length; i++) {
        board[coordinates.xCoordinate - i][coordinates.yCoordinate] =
          Ship.shipNumberEquivalent;
      }
